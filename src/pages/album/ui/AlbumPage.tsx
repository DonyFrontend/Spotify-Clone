    ...
    <div className="bg-[#333333] routeHeight p-3 flex flex-col gap-y-3">
      ...
      <header className="flex gap-x-6 items-end h-[27svh]">
        ...
        <div className="text-white">
          ...
          <div className="flex gap-x-2 items-center">{data.artists.map((item, index) => <p title={t("go_to_artist")} className="text-[18px] hover:underline cursor-pointer" key={index}>{item.name} </p>)} {data.total_tracks} {t("tracks")}</div>
        </div>
      </header>
      <hr className="pb-3" />
      <div className="flex justify-between gap-x-3 items-center">
        <img src={playButton} alt="Error!" className="cursor-pointer" />
        <p className="text-white text-[20px] font-normal cursor-pointer">{t("copy_link")}</p>
      </div>
      <div className="w-full flex justify-center pb-3">
        <div className="flex flex-col gap-y-5 w-[97%]">
          {data.tracks.items.map((item, index) => <Tracks key={index} duration_ms={item.duration_ms} href={item.href} name={item.name} type={item.type} artists={item.artists} disc_number={item.disc_number} track_number={item.track_number} />)}
        </div>
      </div>
